...
if ($jobId) {
  ...
  if (!$job) {
    $_SESSION['error_message'] = "الوظيفة غير موجودة أو لا تملك صلاحية التعديل عليها";
    header('Location: my-job-posts.php');
    exit();
  }
} else {
  $_SESSION['error_message'] = "لم يتم تحديد وظيفة للتعديل";
  header('Location: my-job-posts.php');
  exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  try {
    $stmt = $conn->prepare("UPDATE job_posts SET
        job_title = ?,
        industry = ?,
        vacancies = ?,
        country = ?,
    ...
    $_SESSION['success_message'] = "تم تحديث إعلان الوظيفة بنجاح!";
    header('Location: my-job-posts.php');
    exit();

  } catch (PDOException $e) {
    $error = "حدث خطأ أثناء محاولة تحديث الوظيفة: " . $e->getMessage();
  }
}

// جلب المهارات كـ array
...
<head>
  ...
  <title>التوظيف الذكي - تعديل الوظيفة</title>
  <link href="../css/bootstrap.min.css" rel="stylesheet">
  <link href="../css/fontawesome/css/all.css" rel="stylesheet">
  <link rel="stylesheet" href="css/employer-style.css">
  <!-- استخدم نفس الأنماط من add-job-post.php -->
  <style>
    /* نفس الأنماط الموجودة في add-job-post.php */
    section {
...
<body>
  ...
  <section>
    <div>
      ...
      <div>
        <form action="edit-job-post.php?id=<?php echo $jobId; ?>" method="POST" id="jobPostForm">
          <!-- المسمى الوظيفي -->
          <div class="form-group d-flex flex-column">
            <label class="font-weight-bold">المسمى الوظيفي</label>
            <input type="text" name="job_title" id="jobTitle" class="form-input"
            placeholder="مثل: محاسب" value="<?php echo htmlspecialchars($job['job_title']); ?>">
          ...
          <div class="form-group">
            ...
            <div class="select-wrapper">
              ...
              placeholder="اختر قطاع الوظيفة" readonly
              value="<?php echo htmlspecialchars($job['industry']); ?>"
              onclick="openSelect('industry')">
              <input type="hidden" id="industry" name="industry" value="<?php echo htmlspecialchars($job['industry']); ?>">
            </div>
          ...
          <div class="form-group">
            ...
            min="1" value="<?php echo htmlspecialchars($job['vacancies'] ?? 1); ?>"
            placeholder="أدخل عدد الشواغر">
          ...
          <div class="form-group">
            ...
            <div class="select-wrapper">
              ...
              placeholder="اختر بلد" readonly
              value="<?php echo htmlspecialchars($job['country']); ?>"
              onclick="openSelect('country')">
              <input type="hidden" id="country" name="country" value="<?php echo htmlspecialchars($job['country']); ?>">
            </div>
          ...
          <div class="form-group">
            ...
            <div class="select-wrapper">
              ...
              placeholder="اختر مدينة" readonly
              value="<?php echo htmlspecialchars($job['city']); ?>"
              onclick="openSelect('city')">
              <input type="hidden" id="city" name="city" value="<?php echo htmlspecialchars($job['city']); ?>">
            </div>
          ...
          <div class="form-group">
            ...
            <div class="salary-range">
              <input type="number" class="form-input" placeholder="الحد الأدنى (ريال)"
              id="minSalary" name="min_salary" value="<?php echo htmlspecialchars($job['min_salary']); ?>">
              <input type="number" class="form-input" placeholder="الحد الأقصى (ريال)"
              id="maxSalary" name="max_salary" value="<?php echo htmlspecialchars($job['max_salary']); ?>">
            </div>
          ...
          <!-- حالة الوظيفة -->
          <div class="form-group">
            <label for="status" class="font-weight-bold">حالة الوظيفة</label>
            <select class="status-select" id="status" name="status">
              <option value="active" <?php echo $job['status'] == 'active' ? 'selected' : ''; ?>>نشطة</option>
              <option value="inactive" <?php echo $job['status'] == 'inactive' ? 'selected' : ''; ?>>غير نشطة</option>
              <option value="filled" <?php echo $job['status'] == 'filled' ? 'selected' : ''; ?>>تم شغلها</option>
            </select>
          </div>
          ...
          <div class="form-group">
            <div class="skills-container w-100">
              <div class="skills-tags" id="skillsTagsContainer">
                ...
                <div class="skill-tag">
                  <span class="skill-tag-remove" onclick="removeSkill('<?php echo htmlspecialchars($skill); ?>')">&times;</span>
            ...
            <input type="hidden" id="skillsInput" name="skills" value="<?php echo htmlspecialchars(json_encode($skillsArray)); ?>">
          </div>
          ...
          <div class="form-group">
            ...
            <div class="select-wrapper">
              <select class="normal-select w-100" style="border: 1px solid #ddd;" id="educationLevel" name="education_level">
                ...
                <option value="no qualification required" <?php echo $job['education_level'] == 'no qualification required' ? 'selected' : ''; ?>>no qualification required</option>
                <option value="High school diploma" <?php echo $job['education_level'] == 'High school diploma' ? 'selected' : ''; ?>>High school diploma</option>
                <option value="diploma" <?php echo $job['education_level'] == 'diploma' ? 'selected' : ''; ?>>diploma</option>
          ...
          <div class="form-group">
            ...
            <div class="select-wrapper">
              <select class="normal-select w-100" id="experienceYears" name="experience_years">
                <option value="" disabled>اختر سنوات الخبرة</option>
                <option value="No experience required" <?php echo $job['experience_years'] == 'No experience required' ? 'selected' : ''; ?>>No experience required</option>
                <option value="Less than 1 year" <?php echo $job['experience_years'] == 'Less than 1 year' ? 'selected' : ''; ?>>Less than 1 year</option>
                <option value="1-3  years" <?php echo $job['experience_years'] == '1-3  years' ? 'selected' : ''; ?>>1-3  years</option>
                <option value="3-5  years" <?php echo $job['experience_years'] == '3-5  years' ? 'selected' : ''; ?>>3-5  years</option>
                <option value="5-10  years" <?php echo $job['experience_years'] == '5-10  years' ? 'selected' : ''; ?>>5-10  years</option>
                <option value=" More than 10  years" <?php echo $job['experience_years'] == ' More than 10  years' ? 'selected' : ''; ?>> More than 10  years</option>
              </select>
            </div>
          </div>
          <hr>
          ...
          <div class="form-group">
            <textarea id="jobDescription" class="job-description" name="job_description"
              placeholder="اضف وصفاً وظيفياً"><?php echo htmlspecialchars($job['job_description']); ?></textarea>
          </div>

          <!-- الازرار -->
          <div class="d-flex justify-content-between w-100 p-3">
            <a href="my-job-posts.php" class="btn btn-outline-secondary">
              رجوع
            </a>
            <div>
              <button type="submit" id="publishBtn" class="btn btn-primary">
                <span class="mx-3">حفظ التعديلات</span>
  ...
  <script>
    ...
    let selectedSkills = <?php echo json_encode($skillsArray); ?>;
    ...
    // تحديث المهارات عند تحميل الصفحة
